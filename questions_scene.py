...
game_items = [
    ...
    {'название': 'Яблоко', 'изображение': 'apple.png', 'свойства': {'живое': ['живое', 'не живое'], 'цвет': 'красный', 'тип': 'фрукт'}},
    ...
    {'название': 'Арбуз', 'изображение': 'watermelon.png', 'свойства': {'живое': ['живое', 'не живое'], 'цвет': 'зеленый', 'тип': 'фрукт'}},
    {'название': 'Банан', 'изображение': 'banana.png', 'свойства': {'живое': ['живое', 'не живое'], 'цвет': 'желтый', 'тип': 'фрукт'}},
    ...
    {'название': 'Кактус', 'изображение': 'cactus.png', 'свойства': {'живое': ['живое', 'не живое'], 'цвет': 'зеленый', 'тип': 'растение', 'колючесть': 'колючее'}},
    ...
    {'название': 'Игрушечная собака', 'изображение': 'dog_toy.png', 'свойства': {'живое': 'не живое', 'цвет': 'коричневый', 'тип': 'животное', 'умеет лаять': 'умеет лаять'}},
    {'название': 'Орёл', 'изображение': 'eagle.png', 'свойства': {'живое': 'живое', 'цвет': 'коричневый', 'тип': 'птица'}},
    {'название': 'Лимон', 'изображение': 'limon.png', 'свойства': {'живое': ['живое', 'не живое'], 'цвет': 'желтый', 'тип': 'фрукт', 'кислота': 'кислый'}},
    {'название': 'Апельсин', 'изображение': 'orange.png', 'свойства': {'живое': ['живое', 'не живое'], 'цвет': 'оранжевый', 'тип': 'фрукт'}},
...
class QuestionsScene(Scene):
        ...
        for item in self.items:
            ...
            for key, value in item['свойства'].items():
                if self.questions.get(key) == value or key == 'живое' and self.questions.get(key):
                    continue
                bot_basic = ['цвет', 'плавает', 'умеет говорить', 'высота', 'что ест', 'кислота', 'умеет лаять']
                if key not in bot_basic:
                    current_question = f'Это {value}?'
                elif key == 'цвет':
                    current_question = f'Оно {value[0:-2]}ого цвета?'
                elif key == 'кислота':
                    current_question = f'Оно {value[:-2]}ое?'
                else:
    ...
    def check(self, key, value, answer):
        for item in self.items:
            if (answer and item['свойства'].get(key) != value) and not isinstance(item['свойства'].get(key), list):
                self.items.remove(item)
                print(*self.items)
                self.check(key, value, answer)
            elif (not answer and item['свойства'].get(key) == value) and not isinstance(item['свойства'].get(key), list):
                self.items.remove(item)
                print(*self.items)
                self.check(key, value, answer)
            elif isinstance(item['свойства'].get(key), list) and answer and not value in item['свойства'].get(key):
                self.items.remove(item)
                print(*self.items)
