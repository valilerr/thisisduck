import pygame

from result_scene import ResultScene
from start_scene import StartScene
from scene_manager import SceneManager
from start_menu import StartMenu
from questions_scene import QuestionsScene
from ui_elements.button import Button

scene_manager = SceneManager()
scene_manager.add_scene('StartMenu', StartMenu)
scene_manager.add_scene('StartScene', StartScene)
scene_manager.add_scene('QuestionsScene', QuestionsScene)
scene_manager.add_scene('ResultScene', ResultScene)

class ThisIsDuck:
    def __init__(self):
        pygame.init()
        self.screen = pygame.display.set_mode((800, 600), pygame.NOFRAME)
        pygame.display.set_caption('ThisIsDuck!')
        self.running = True
        scene_manager.start_scene('StartMenu')
        self.header = pygame.Rect(2, 2, 796, 25)
        self.title = pygame.font.Font('fonts/Unbounded-Medium.ttf', 16).render('This Is Duck!', True, (200, 200, 200))
        self.close_btn = Button((774, 2), (24, 24), 'X')

        while self.running:
            pygame.display.update()

            scene_manager.draw(self.screen)
            pygame.draw.rect(self.screen, (50, 50, 50), self.header, border_radius=8)
            self.screen.blit(self.title, (4, 6))
            self.close_btn.draw(self.screen)
            self.close_btn.check_hover(pygame.mouse.get_pos())

            for event in pygame.event.get():
                if self.close_btn.is_clicked(pygame.mouse.get_pos(), event):
                    self.running = False

                scene_manager.handle_events(event, scene_manager)

ThisIsDuck()